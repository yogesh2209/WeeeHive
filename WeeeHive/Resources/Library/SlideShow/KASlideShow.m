...
- (void) emptyAndAddImages:(NSArray *)imagess
{
    [self.images removeAllObjects];
    _currentIndex = 0;
    for (UIImage *image in imagess){
        [self addImage:image];
    }
}
...
- (void) handleSwipe:(id)sender
{
    ...
    if (gesture.direction == UISwipeGestureRecognizerDirectionLeft)
    {
       ...
       // NSLog(@"%lu",(unsigned long)self.images.count);
        NSUInteger temp= _currentIndex + 1;
        
        
        
        if (temp==self.images.count) {
            _complition(temp);
...
//method definition
- (void)getSelectedIndex:(complition)block {
